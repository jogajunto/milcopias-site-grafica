...
export function PayloadImage({ image, className, width, height, loading = "lazy" }: PayloadImageProps) {
  ...
  return <Image key={image.id} className={className} src={image.url!} width={width ? width : image.width!} height={height ? height : image.height!} alt={image.alt || "..."} loading={loading} />;
}
